...
class CampVehicleAllocatorILP:
    ...
    def solve_ilp_allocation(self, time_limit=300):
        ...
        # Check if we have odd-numbered dorms
        ...
        if odd_dorms:
            ...
            print("  This might make it impossible to avoid some isolated individuals.")
        else:
            print("  ✓ All dorms have even numbers - should be possible to avoid isolated individuals")
        ...
        # 8. Soft constraint: Try to ensure minimum group size >= 2
        # Create violation variables for cases where we must have a group size of 1
        violations = {}
        for vehicle in usable_vehicles:
            var_name = f"violation_{vehicle}".replace(" ", "_").replace("(", "").replace(")", "").replace("-", "_")
            violations[vehicle] = pulp.LpVariable(var_name, lowBound=0, cat='Integer')
            
            # If vehicle is used, then m[vehicle] + violations[vehicle] >= 2
            # This means m[vehicle] >= 2 - violations[vehicle]
            # If violations[vehicle] = 0, then m[vehicle] >= 2 (our goal)
            # If violations[vehicle] = 1, then m[vehicle] >= 1 (allowing singles if necessary)
            constraint_name = f"Min_Group_Two_{vehicle}".replace(" ", "_").replace("(", "").replace(")", "").replace("-", "_")
            model += m[vehicle] + violations[vehicle] >= 2 * z[vehicle], constraint_name
        ...
        # OBJECTIVE: Maximize weighted scores + single dorm bonuses (cars only) + gender cohesion - heavy penalty for violations
        single_dorm_bonus_weight = 20  # Significant bonus for single-dorm cars (not minibuses)
        gender_cohesion_weight = 10  # Moderate bonus for single-gender mixed-dorm vehicles
        violation_penalty = 1000  # Huge penalty for allowing group size of 1
        ...
        objective = pulp.lpSum(
            ...
            single_dorm_bonus_weight * single_dorm_bonus[vehicle] 
            for vehicle in single_dorm_bonus  # Only includes non-minibuses
        ) - pulp.lpSum(
            violation_penalty * violations[vehicle]
            for vehicle in usable_vehicles
        )
        ...
        if self.people_gender and gender_cohesion_bonus:
            objective += pulp.lpSum(
                gender_cohesion_weight * gender_cohesion_bonus[vehicle]
        ...
        print("\nScoring breakdown:")
        print("  Min group sizes: 1→1pt, 2→4pts, 3→9pts, 4→16pts, 5→25pts, etc.")
        print("  Single-dorm bonus: 20pts per car (not minibuses)")
        if self.people_gender:
            print("  Gender cohesion bonus: 10pts for mixed-dorm vehicles with single gender")
        print("  Isolation penalty: -1000pts per vehicle with min group size of 1")
        
        # Check for violations in the solution
        total_violations = 0
        if 'violations' in locals():
            for vehicle in usable_vehicles:
                if vehicle in violations and pulp.value(violations[vehicle]) > 0.5:
                    total_violations += 1
        
        if total_violations > 0:
            print(f"\n⚠️  WARNING: {total_violations} vehicles have isolated individuals (min group size = 1)")
    ...
    def print_ilp_solution(self, allocation):
        ...
        for (vehicle, driver), passengers in sorted_allocation:
            ...
            if is_single_dorm and 'Minibus' not in vehicle:
                vehicle_score += 20
                bonus_info.append("Single dorm +20")
            ...
            if self.people_gender and not is_single_dorm and len(gender_counts) == 1:
                vehicle_score += 10
                bonus_info.append("Gender cohesion +10")
